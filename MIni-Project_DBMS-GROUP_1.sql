...
-- Answer:
select IPL_BIDDER_DETAILS.BIDDER_ID as Bidder_ID, IPL_BIDDER_DETAILS.BIDDER_NAME as Bidder_Name,
(select count(*) from IPL_BIDDING_DETAILS 
where IPL_BIDDING_DETAILS.BID_STATUS = 'Won' and IPL_BIDDING_DETAILS.BIDDER_ID = IPL_BIDDER_DETAILS.BIDDER_ID )*100 / 
(select IPL_BIDDER_POINTS.NO_OF_BIDS from IPL_BIDDER_POINTS
where IPL_BIDDER_POINTS.BIDDER_ID = IPL_BIDDER_DETAILS.BIDDER_ID ) as Percentage_of_Wins
from IPL_BIDDER_DETAILS ,IPL_BIDDING_DETAILS 
group by IPL_BIDDER_DETAILS.BIDDER_ID
order by Percentage_of_Wins desc;

#Insights

#From the output we get to know that Mr.'Megaduta Dheer' is having 100% chance of winning.

-- 2.	Display the number of matches conducted at each stadium with the stadium name and city

-- Answer:
select IPL_STADIUM.STADIUM_ID, IPL_STADIUM.STADIUM_NAME, IPL_STADIUM.CITY, count(IPL_MATCH_SCHEDULE.MATCH_ID)  as COUNT
from IPL_MATCH_SCHEDULE, IPL_STADIUM
where IPL_STADIUM.STADIUM_ID = IPL_MATCH_SCHEDULE.STADIUM_ID
group by IPL_STADIUM.STADIUM_ID;

#Insights

#From the output we can determine count of matches with respect to city and stadium.
...
(SELECT count(*) FROM ipl_match  JOIN ipl_match_schedule on ipl_match.match_id = ipl_match_schedule.match_id
WHERE ipl_match_schedule.stadium_id = ipl_stadium.stadium_id AND (toss_winner = match_winner)) /
(SELECT count(*) from ipl_match_schedule  WHERE ipl_match_schedule.stadium_id = ipl_stadium.stadium_id) * 100 
AS 'Percentage of Wins by teams who won the toss (%)'
FROM ipl_stadium;

#Insights

#From the output we get to know the stadium namely, 'Sawai Mansingh Stadium','Is Bindra Stadium','Wankhede Stadium' has top three percentage of wins.
#In the above three stadiums chances of winning the match is more for the one who wins the toss.
...
#Insights
#From the output we can infer that 'Sunrisers Hyderabad' has more chances of winning among other teams 
...
#Insights
#In this output we can know the win details of each team which helps for further prediction

-- 6.	Display total matches played, total matches won and total matches lost by the team along with its team name.

-- ANSWER:
SELECT IPL_TEAM_STANDINGS.TEAM_ID,
IPL_TEAM.TEAM_NAME,
...
#Insights
#From the output, we infer the 'form' status of all teams
#with that, we came to know chennai super kings is in very good form

-- 7.	Display the bowlers for the Mumbai Indians team.

-- ANSWER:
SELECT IPL_TEAM_PLAYERS.PLAYER_ID,
IPL_TEAM.TEAM_NAME,
IPL_TEAM_PLAYERS.REMARKS,
PLAYER_ROLE,
PLAYER_NAME
FROM IPL_PLAYER
INNER JOIN IPL_TEAM_PLAYERS
ON IPL_TEAM_PLAYERS.PLAYER_ID = IPL_PLAYER.PLAYER_ID
AND PLAYER_ROLE LIKE "%batsman%"
INNER JOIN IPL_TEAM
ON IPL_TEAM.TEAM_ID = IPL_TEAM_PLAYERS.TEAM_ID
AND TEAM_NAME LIKE "%Rajasthan%"
ORDER BY PLAYER_ID;

#Insights
#from the output we can view all the bowlers and their info from the team

-- 8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounders in descending order.

-- ANSWER:
select IPL_TEAM.TEAM_ID, IPL_TEAM.TEAM_NAME, count(IPL_TEAM_PLAYERS.PLAYER_ROLE )  as ALL_Rounder_count
from  IPL_TEAM_PLAYERS, IPL_TEAM
where  IPL_TEAM.TEAM_ID =IPL_TEAM_PLAYERS.TEAM_ID and (IPL_TEAM_PLAYERS.PLAYER_ROLE = 'All-Rounder') 
group by IPL_TEAM.TEAM_ID
having count(IPL_TEAM_PLAYERS.PLAYER_ROLE)>4
order by count(IPL_TEAM_PLAYERS.PLAYER_ROLE) desc;
...
-- ANSWER:
select sum(total_points) as total_bidding_points, bid_status, year(bid_date) as bidding_year from 
ipl_bidder_points as ibp cross join  ipl_team as it cross join ipl_bidding_details as ibd cross join ipl_stadium as ips
where it.team_id=1 and ips.stadium_id=7
group by bid_status
order by bidding_year;

#Insights
#From the output we can reach out the status of the bidders for a particular year along with the points which will be helpful for further prediction
...
-- ANSWER:
SELECT IPL_PLAYER.PLAYER_ID,IPL_PLAYER.PLAYER_NAME FROM (SELECT ipl_team_players.PLAYER_ID,ipl_team_players.PLAYER_ROLE 
FROM ipl_team_players
WHERE PLAYER_ROLE='BOWLER' OR PLAYER_ROLE='ALL-ROUNDER') WHERE IPL_PLAYER.PERFORMANCE_DTLS LIKE '%WKt'
...
-- ANSWER:
select IPL_BIDDER_DETAILS.BIDDER_ID as Bidder_Id, IPL_BIDDER_DETAILS.BIDDER_NAME as Bidder_Name ,
(select count(IPL_MATCH.TOSS_WINNER) from IPL_MATCH 
where IPL_MATCH.TOSS_WINNER = IPL_BIDDING_DETAILS.BID_TEAM )*100/
(select sum(IPL_BIDDER_POINTS.NO_OF_BIDS) from IPL_BIDDER_POINTS) as Percentage_of_Toss_Wins
from IPL_BIDDING_DETAILS, IPL_BIDDER_DETAILS
group by IPL_BIDDER_DETAILS.BIDDER_NAME
order by  Percentage_of_Toss_Wins desc;

select count(toss_winner) from ipl_match

select team_id, toss_winner from ipl_match
where toss_winner=1
...
select tournmt_id, 
tournmt_name, 
datediff(to_date,from_date) as duration_days,
dense_rank() over(order by datediff(to_date,from_date)) as duration_rank from ipl_tournament;
...
-- ANSWER:
SELECT IPL_BIDDER_DETAILS.BIDDER_ID,
IPL_BIDDER_DETAILS.BIDDER_NAME,
YEAR(IPL_BIDDING_DETAILS.BID_DATE) BID_YEAR,
MONTH(IPL_BIDDING_DETAILS.BID_DATE) BID_MONTH,
sum(IPL_BIDDER_POINTS.TOTAL_POINTS)TOTAL_BIDDING_POINTS
FROM IPL_BIDDER_DETAILS
INNER JOIN IPL_BIDDING_DETAILS
    USING (BIDDER_ID)
INNER JOIN IPL_BIDDER_POINTS
    USING (BIDDER_ID)
WHERE YEAR(IPL_BIDDING_DETAILS.BID_DATE)='2017'
GROUP BY 
	1,2,3,4
ORDER BY
   BID_dATE;
   
select bd1.BIDDER_ID,bd1.BIDDER_NAME,year(bd2.bid_date) Year,month(bd2.bid_date) month,sum(bp.TOTAL_POINTS) as total_points
from ipl_bidder_details bd1 join ipl_bidding_details bd2
on bd1.BIDDER_ID = bd2.BIDDER_ID
join ipl_bidder_points bp on bp.BIDDER_ID = bd2.BIDDER_ID
where year(bd2.BID_DATE) = 2017 
group by 1,2,3,4
...
IPL_BIDDER_DETAILS.BIDDER_NAME, 
year(IPL_BIDDING_DETAILS.BID_DATE) as year,
month(IPL_BIDDING_DETAILS.BID_DATE) as month, 
sum(IPL_BIDDER_POINTS.TOTAL_POINTS) as Total_points, 
IPL_BIDDING_DETAILS.BID_DATE 
from IPL_BIDDER_POINTS, IPL_BIDDER_DETAILS, IPL_BIDDING_DETAILS
where IPL_BIDDING_DETAILS.BIDDER_ID =IPL_BIDDER_POINTS.BIDDER_ID
and year(IPL_BIDDING_DETAILS.BID_DATE) = 2017
group by month(IPL_BIDDING_DETAILS.BID_DATE)
order by IPL_BIDDING_DETAILS.BID_DATE ;
...
-- Answer
WITH HIGHEST_BIDDERS AS(
    SELECT 
        ipl_bidder_details.BIDDER_ID, 
        ipl_bidder_details.BIDDER_NAME,
        YEAR(IPL_BIDDING_DETAILS.BID_DATE) BIDDING_year,
        ipl_bidder_points.TOTAL_POINTS AS TOTAL_BIDDING_POINTS,
        
        DENSE_RANK() OVER (
            PARTITION BY YEAR(IPL_BIDDING_DETAILS.BID_DATE)
            ORDER BY ipl_bidder_points.TOTAL_POINTS DESC
        ) BIDDER_rank
    FROM IPL_BIDDER_POINTS    
        INNER JOIN ipl_bidder_details USING (BIDDER_ID)
        INNER JOIN IPL_BIDDING_DETAILS USING (BIDDER_ID)
       
)
SELECT 
    * 
FROM HIGHEST_BIDDERS
WHERE 
    BIDDING_YEAR=2018 and BIDDER_RANK<4 or BIDDER_RANK>13
...
##CREATING FIRST TABLE WITH ATTRIBUTES CALLED STUDENT_DETAILS (PARENT TABLE)
...
CREATE TABLE IF NOT EXISTS Student_details (
    ...
    course varchar(20),
    batch varchar(10),
    project_Group_no int
    );
    DROP TABLE IF EXISTS Student_details;
    
##CREATING SECOND TABLE WITH ATTRIBUTES 

    CREATE TABLE IF NOT EXISTS BKP_Student_details (
    student_id INT AUTO_INCREMENT PRIMARY KEY,
    Name VARCHAR(255) NOT NULL,
    Mail_id varchar(100),
    Mobile_No varchar(20),
...
DROP TABLE IF EXISTS BKP_Student_details;
SELECT * FROM BKP_Student_details;

##DUMPING vALUES IN STUDENT_DETAILS TABLE
INSERT INTO student_details(student_id,Name ,Mail_id,Mobile_No,course,batch,project_Group_no)
VALUES
    ('001','Adhipriyanka A V','adhipriyanka203@gmail.com',9876543210,'PGDSE','online','1'),
    ('002','Aditya','adityasingh02@gmail.com',7654398721,'PGDSE','offline','2'),
    ('003','Ali','mirali5007@gmail.com',8890564371,'ML&BA','online','3'),
    ('004','Alekhya','Alekhyahyd506@gmail.com',7865467388,'AI&CC','offline','4'),
    ('005','Sonu Thalia','sonuthalia@gmail.com',8965765123,'PGDSBA','online','5');

##CREATING A TRIGGER TO INSERT AND UPDATE VALUES IN BKP_Student_details

DELIMITER $$

CREATE TRIGGER before_BKP_Student_details_insert
BEFORE INSERT 
ON Student_details
FOR EACH ROW
BEGIN
    ...
    SELECT COUNT(*) 
    INTO rowcount
    FROM BKP_Student_details;
    
    IF rowcount > 0 THEN
        UPDATE BKP_Student_details
        SET OLD.STUDENT_ID=NEW.STUDENT_iD;
    ELSE
        INSERT INTO BKP_Student_details(student_id,Name ,Mail_id,Mobile_No,course,batch,project_Group_no)
        VALUES(NEW.student_id,NEW.Name ,NEW.Mail_id,NEW.Mobile_No,NEW.course,NEW.batch,NEW.project_Group_no);
    END IF; 
...
SHOW triggers;
INSERT INTO BKP_Student_details(student_id,Name ,Mail_id,Mobile_No,course,batch,project_Group_no)
VALUES('006','WAJID SHERIFF','WAJIDSHERIFF@GMAIL.COM','9675380883','MACHINELEARNING','OFFLINE','7');


DROP TRIGGER before_BKP_Student_details_insert ;
